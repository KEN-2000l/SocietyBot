...
class MessageHook(Cog):
    ...
    @command(aliases=['say'])
    async def send(self, ctx: Context, *, text=''):
        if (text == '') and (ctx.message.attachments is None):
            return
        if len(text) > 2000:
    ...
    @command()
    async def cleanhooks(self, ctx: Context, *args):
        if args:
            return
        if not ctx.guild:
            return
